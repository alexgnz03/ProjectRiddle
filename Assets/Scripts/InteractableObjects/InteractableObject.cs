...
{
    public InteractableObjectDialogues dialogueData;
    ...
    private enum correctObjectReaction { NoReaction, DestroyObject }
    [SerializeField]
    private correctObjectReaction correctObjectCase = correctObjectReaction.NoReaction;
    private bool correctObjectShowed = false;
    ...
    public void ShowObject(GameObject showedObject)
    {
        ...
        if (showedObject != null)
        {
            ...
            if (showedObject.GetComponent<Item>().itemData == correctObject.GetComponent<Item>().itemData)
            {
                ...
                else if (correctObjectCase == correctObjectReaction.DestroyObject)
                {
                    correctObjectShowed = true;
                }
                Interact();
            }
            else
    ...
    public void EndDialogue()
    {
        ...
        if (correctObjectCase == correctObjectReaction.DestroyObject && correctObjectShowed == true)
        {
            InteractionDetector detector = FindObjectOfType<InteractionDetector>();
            if (detector != null)
            {
                detector.ClearCurrentInteractable(this);
            }
